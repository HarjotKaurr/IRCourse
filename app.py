...
if selected == "🏠 Home":
    ...
    student_records = db_handler.fetch_data("SELECT * FROM Student_Details")
    df_students = pd.DataFrame(student_records, columns=["Admn_No", "S_name", "Class", "Sec", "Per", "DOB", "Bloodgp", "Phone_no"])
    ...
    df_parents = pd.DataFrame(parent_records, columns=["Admn_No", "P_ID", "P_Name", "contact_no", "Address"])
    ...
    df_fees = pd.DataFrame(fee_records, columns=["Admn_No", "Fee_Type", "FeePaid"])
    ...
    df_marks = pd.DataFrame(marks_records, columns=["Admn_No", "Subject1", "Subject2", "Subject3"])
...
#     def add_fee(self, admn_no, fee_paid):
#         query = """
#         INSERT INTO Fee_Details (Admn_No, Fee_Paid)
#         VALUES (%s, %s)
#         """
#         data = (admn_no, fee_paid)
#         try:
#             self.db_handler.execute_query(query, data)
#             st.success("✅ Fee details added successfully!")
#         except Exception as e:
...
#     st.subheader("💰 Fee Details")
#     fee_records = db_handler.fetch_data("SELECT * FROM Fee_Details")
#     df_fees = pd.DataFrame(fee_records, columns=["Admn_No", "Fee_Paid"])
#     st.dataframe(df_fees, use_container_width=True)
...
#     st.title("💰 Add Fee Details")
#     with st.form("add_fee_form"):
#         admn_no = st.text_input("📌 Admission Number")
#         fee_paid = st.selectbox("📌 Fee Paid?", ["Y", "N"])
#         submit = st.form_submit_button("✅ Add Fee")
#         if submit:
#             fee.add_fee(admn_no, fee_paid)
...
#     selected = option_menu(
#         "Student Management",
#         ["🏠 Home", "📚 Add Student", "👨‍👩‍👦 Add Parent", "💰 Add Fees", "📊 Add Marks", "🔍 Search Student", "❌ Delete Student"],
#         icons=["house", "plus", "user-plus", "dollar-sign", "bar-chart", "search", "trash"],
#         menu_icon="cast",
#         default_index=0
#     )
...
#         st.subheader("👨‍👩‍👦 Parent Details")
#         parent_records = db_handler.fetch_data("SELECT * FROM Parent_Details")
#         df_parents = pd.DataFrame(parent_records, columns=["Admn_No", "P_ID", "Parent Name", "Contact No", "Address"])
#         st.dataframe(df_parents, use_container_width=True)

#         # Fee Details
#         st.subheader("💰 Fee Details")
#         fee_records = db_handler.fetch_data("SELECT * FROM Fee_Details")
#         df_fees = pd.DataFrame(fee_records, columns=["Admn_No", "Fee Type", "Fee Paid"])
#         st.dataframe(df_fees, use_container_width=True)
...
#         if submit:
#             query = "INSERT INTO Parent_Details (Admn_No, P_ID, P_name, Contact_No, Address) VALUES (%s, %s, %s, %s, %s)"
#             db_handler.execute_query(query, (admn_no, p_id, parent_name, contact_no, address))
#             st.success("✅ Parent details added successfully!")

# # **💰 Add Fees Details**
# elif selected == "💰 Add Fees":
#     st.title("💰 Add Fee Details")
#     with st.form("add_fees_form"):
#         admn_no = st.text_input("📌 Admission Number")
#         fee_type = st.selectbox("📌 Fee Type", ["Annually", "Quaterly", "Monthly"])
#         fee_paid = st.number_input("📌 Fee Paid", ["Y", "N"])
#         submit = st.form_submit_button("✅ Add Fee")

#         if submit:
#             query = "INSERT INTO Fee_Details (Admn_No, Fee_Type, Fee_Paid) VALUES (%s, %s, %s)"
#             db_handler.execute_query(query, (admn_no, fee_type, fee_paid))
#             st.success("✅ Fee details added successfully!")

# # **📊 Add Marks Details**
# elif selected == "📊 Add Marks":
#     st.title("📊 Add Marks Details")
#     with st.form("add_marks_form"):
#         admn_no = st.text_input("📌 Admission Number")
#         subject1 = st.number_input("📌 Marks for Subject 1", min_value=0, max_value=100)
#         subject2 = st.number_input("📌 Marks for Subject 2", min_value=0, max_value=100)
#         subject3 = st.number_input("📌 Marks for Subject 3", min_value=0, max_value=100)
#         submit = st.form_submit_button("✅ Add Marks")

#         if submit:
#             query = "INSERT INTO Marks (Admn_No, Subject1, Subject2, Subject3) VALUES (%s, %s, %s, %s)"
#             db_handler.execute_query(query, (admn_no, subject1, subject2, subject3))
#             st.success("✅ Marks details added successfully!")

# # **🔍 Search Student**
# elif selected == "🔍 Search Student":
#     st.title("🔍 Search Student Record")
#     admn_no = st.text_input("📌 Enter Admission Number")
#     search = st.button("🔍 Search")

#     if search:
#         query = "SELECT * FROM Student_Details WHERE Admn_No = %s"
#         result = db_handler.fetch_data(query, (admn_no,))
#         df = pd.DataFrame(result, columns=["Admn_No", "Name", "Class", "Sec", "Percentage", "DOB", "Blood Group", "Phone No"])
